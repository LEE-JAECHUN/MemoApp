...
class DataSync {
    ...
    func downloadBackupData() {
        ...
        let get = Alamofire.request(url, method: .post, parameters: nil, encoding: JSONEncoding.default, headers: header)
        // 4. 응답 처리
        get.responseJSON(completionHandler: { res in
            // 5. 응답 결과 잘못되었거나 list 항목이 없을 경우에는 잘못된 응답이므로 그대로 종료한다
            ...
            for item in list {
                ...
                // 7. MemoMo 타입의 관리 개체 인스턴스를 생성하고, 각 속성에 값을 대입한다
    ...
    // Memo 엔티티에 저장된 모든 데이터 중에서 동기화 되지 않은 것을 찾아 업로드 한다
    func uploadData() {
        // 1. 요청 객체 생성
